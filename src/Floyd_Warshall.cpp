...
// Tiled implementation of Floyd_Marshall algorithm, with adapted memory layout in blocs
// blocSize1D = bloc width
std::vector<std::vector<unsigned short>> floyd_warshall_seq_tiled_layout2(std::vector<std::vector<unsigned short>> adjacency_matrix, int blocSize1D) {
    int V = adjacency_matrix[0].size();
    int blocsPerWidth = V / blocSize1D;
    std::vector<std::vector<unsigned short>> graph = graphToBlocLayout(adjacency_matrix, blocSize1D, false);
    int i, ii, j, jj, k;
    ...
    for (int bloc = 0; bloc < blocsPerWidth; bloc++) {
        ...
        for (k = 0; k < blocSize1D; k++) {
            ...
            for (i = 0; i < blocSize1D; i++) {
                for (j = 0; j < blocSize1D; j++) {
                    if (graph[i*blocSize1D + j][numQuad] > graph[i*blocSize1D + k][numQuad] + graph[k*blocSize1D + j][numQuad])
                    {                                                                                                         
                        graph[i*blocSize1D + j][numQuad] = graph[i*blocSize1D + k][numQuad] + graph[k*blocSize1D + j][numQuad];
                    }
                }
            }
        }
        ...
        for (k = 0; k < blocSize1D; k++) {
            ...
            for (i = 0; i < blocSize1D; i++) {
                for (j = 0; j < blocsPerWidth; j++) {
                    ...
                    int numCurrentQuad = numQuadLeft + j;
                    for (jj = 0; jj < blocSize1D; jj++) {
                        if (graph[i*blocSize1D+jj][numCurrentQuad] > graph[i*blocSize1D+k][numQuad] + graph[k*blocSize1D+jj][numCurrentQuad])
                        {                                                                                                                   
                            graph[i*blocSize1D+jj][numCurrentQuad] = graph[i*blocSize1D+k][numQuad] + graph[k*blocSize1D+jj][numCurrentQuad];
                        }
                    }
                }
            }
            ...
            for (i = 0; i < blocsPerWidth; i++) {
                ...
                for (ii = 0; ii < blocSize1D; ii++) {
                    for (j = 0; j < blocSize1D; j++) {
                        if (graph[ii*blocSize1D+j][numCurrentQuad] > graph[ii*blocSize1D+k][numCurrentQuad] + graph[k*blocSize1D+j][numQuad])
                        {                                                                                                                   
                            graph[ii*blocSize1D+j][numCurrentQuad] = graph[ii*blocSize1D+k][numCurrentQuad] + graph[k*blocSize1D+j][numQuad];
                        }
                    }
                }
            }
        ...
        for (i = 0; i < blocsPerWidth; i++) {
            ...
            for (ii = 0; ii < blocSize1D; ii++) {
                for (j = 0; j < blocsPerWidth; j++) {
                    ...
                    int numQuadKJ = bloc * blocsPerWidth + j;
                    for (k = 0; k < blocSize1D; k++) {
                        int distIK = graph[ii*blocSize1D+k][numQuadIK];
                        for (jj = 0; jj < blocSize1D; jj++) {
                            if (graph[ii*blocSize1D+jj][numQuadIJ] > distIK + graph[k*blocSize1D+jj][numQuadKJ])
                            {                                                                                                             
                                graph[ii*blocSize1D+jj][numQuadIJ] = distIK + graph[k*blocSize1D+jj][numQuadKJ];
                            }
                        }
                    }
                }
    ...
    return blocLayoutToGraph(graph, V, blocSize1D, false);
}
